...
class AboutUsElementController extends Controller
{
    ...
    protected function validateImage(Request $request)
    {
        $request->validate([
            'element' => 'required|string',
            'data-image' => [
                'required_if:element,Image',
                function ($attribute, $value, $fail) {
                    // Check if the value is a file or a valid image URL/filename
                    if (!filter_var($value, FILTER_VALIDATE_URL) && !is_file(public_path('uploads/' . $value))) {
                        $fail('The ' . $attribute . ' must be a valid image file or a valid image path.');
                    }
                }
            ],
        ]);
