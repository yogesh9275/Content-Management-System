...
class AboutUsElementController extends Controller
{
    ...
    {
        // Find the element or throw an exception if not found
        $element = AboutUsElement::findOrFail($id);

        // Check if the 'data' column contains a path starting with 'uploads/'
        if (strpos($element->data, 'uploads/') === 0) {
            // Generate the full file path
            $filePath = base_path($element->data);
