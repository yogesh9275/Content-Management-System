        ...
        <form action="{{ route('galleries.update', $gallery->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select name="category" id="category" class="form-select" required>
                    ...
                    <option value="meet-leader" {{ $gallery->category == 'Celebrations' ? 'selected' : '' }}>Celebrations</option>
                    <option value="Religional" {{ $gallery->category == 'Religional' ? 'selected' : '' }}>Religional</option>
                    <option value="public-function" {{ $gallery->category == 'Promotion' ? 'selected' : '' }}>Promotion</option>
                </select>
            </div>
            ...
            <div class="d-flex justify-content-between">
                <a id="back-btn" href="{{ route('galleries.index') }}" class="btn btn-secondary">Back</a>
                <button id="update-btn" type="submit" class="btn btn-warning">Update Image</button>
            </div>
        </form>
    </div>
