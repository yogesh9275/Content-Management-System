                ...
                <form action="{{ route('homepage.update', $homePage->id) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select" name="element" id="element" onchange="toggleDivs()" required>
                            ...
                            <option value="vision-image" {{ $homePage->element == 'vision-image' ? 'selected' : '' }}>
                                Vision-Image</option>
                            <!-- Set the value based on element -->
                            <option
                                value="{{ Str::startsWith($homePage->element, 'selected-image-') ? $homePage->element : 'slider-image' }}"
                                {{ Str::startsWith($homePage->element, 'selected-image-') ? 'selected' : '' }}>
                                {{ Str::startsWith($homePage->element, 'selected-image-') ? 'Selected Image' : 'Slider-Image' }}
                            </option>
                        </select>
                    ...
                    @if (
                        ...
                        <div class="mt-3" id="image-preview">
                            <img id="preview-img" src="{{ url( $homePage->data) }}" class="img-thumbnail mb-2"
