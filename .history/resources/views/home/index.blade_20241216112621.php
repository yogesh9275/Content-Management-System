                ...
                <div class="card my-4" style="width: 100%;">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center border p-3 rounded">
                            <h5 class="card-title text-dark fw-bold" style="margin-bottom: 0">{{ $homePage->element }}</h5>
                            <div class="d-flex justify-content-end gap-2">
                                <form action="{{ route('about-us.edit', $homePage->id) }}" method="GET" class="d-inline">
                                    <button id="edit-btn" type="submit" class="btn btn-warning btn-sm d-flex align-items-center">
                                        <x-simpleline-pencil class="icon-size" />
                                    </button>
                                </form>
                                <form action="{{ route('about-us.destroy', $homePage->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
